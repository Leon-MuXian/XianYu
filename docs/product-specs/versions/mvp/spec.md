...
| O-START | 店长端 / 开始使用 | 首次登录无门店 | 已登录 | 新用户免费试用告示、当前试用天数、开店清单、门店资料/营业时间/履约资源三个必填项完成状态、创建门店按钮禁用/可用状态、必填项完成后的门店展示预览 | 我知道了/点击必填项/创建门店 | 页面固定首屏且只展示三个可操作项；必填项未完成时创建门店禁用并提示剩余项；当前默认试用天数为 30 天，由平台配置决定 |
